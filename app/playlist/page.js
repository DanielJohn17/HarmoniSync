...
const Page = () => {
  ...
  const fetchPlaylist = async () => {
    const res = await fetch(
      `http://127.0.0.1:5000/api/v1/users/${dummyUserID}/playlists`
    );
  ...
  const fetchPlaylistSongs = async (playlist_id) => {
    const response = await fetch(
      `http://127.0.0.1:5000/api/v1/users/${dummyUserID}/playlists/${playlist_id}/tracks`
    );
  ...
  const postNewPlaylist = async (newPlaylistName) => {
    ...
    const response = await fetch(
      `http://127.0.0.1:5000/api/v1/users/${dummyUserID}/playlists/create_playlist`,
      {
        method: "POST",
      ...
      <div className="playlist-container">
        <div className="playlist-titles">
          <div className="playlist-header">
            <h3>Playlists</h3>
            <button className="playlist-add" onClick={handleAdd}>
              Add
            </button>
          </div>
